            ...
            <div className="backdrop-blur-sm bg-gray-100 rounded-sm">
              ...
              <div className="space-y-3 p-4 mt-4">
                {Array.from({ length: 6 }, (_, i) => i + 1)
                  .reverse()
                  .map((floor) => {
                    const binInfo = rack.binData?.find(
                      (bin) => bin.floor === floor
                    );
                    const hasItems =
                      binInfo && binInfo.lotList && binInfo.lotList.length > 0; // 수정된 부분
                    ...
                    return (
                      ...
                      >
                        ...
                        {binInfo?.lotList.map((lot) => (
                          <div
                            key={lot.lotId}
                            className="mt-2 bg-white/70 rounded-sm p-2 text-sm text-gray-600"
                          >
